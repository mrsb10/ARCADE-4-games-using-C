...
int isstackarrayascending(struct stack* stack) {
    ...
    for (int i = 0; i < stack->top; ++i) {
        if (stack->array[i] > stack->array[i + 1]) {
            return 0; //it is not in ascending
        }
    }
    return 1; //it is in ascending order
}


int towerofhanoi(int numdisks, struct stack* source, struct stack* auxiliary, struct stack* target) { //three stacks just like tower of hanoi
    // int totalmoves = (1 << numdisks) - 1; // Total moves required


    for (int i = numdisks; i > 0; --i) { //no of disks to put in pegs
        push(source, i);
    }
...
int generate() {
    return rand() % 10 + 1;  //to generate random number using srand()
}
...
int play_tic_tac_toe(char player1[20], char player2[20]) {
    ...
    do {
        ...
        while (!valid_move) {
            printf("%s, enter your move (1-9): ", (current_player == 'X') ? player1 : player2);
            scanf("%d", &move);
